...
- 新类型注册
如果模块实例属于新的类型，即项目中还没有注册过相同类型的实例，我们需要进行模块类型的注册
...
- YAML 清单文件（例如 modules/<mod_id>/MANIFEST.yaml），捕获模块的关键元数据（其 module_type、对其他模块的任何依赖、作者等）。
  - 模块根目录中的核心AI文档：
    ...
    - `ABILITY.md`：实例可以直接或间接调用的分装好的能力，用于告知AI可用工具，从而提高代码的规范程度和可控程度。
    - `ROUTING.md`：实例相关的顶层知识路由，给出实例开发过程中，各情景下任务编排可能需要的路由，最终指向知识文档。
  - 模块内容的标准子目录，例如：
...
│   │   ├── task-1-errors.md    # 错误经验，避免再犯
│   │   └── task-1-tasks.md     # 任务完成情况和进度
│   │   └── task-1-details.md   # 记录任务
│   └── task-2/
│       └── ...
...
从AI实际参与开发的角度出发，AI阅读入口越明确，我们就能更加准确的把文档放到合适的目录，从而提高AI获取必要信息和任务编排的效率。
