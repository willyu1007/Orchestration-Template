...
1. 找到正确的能力路由文档`ABILITY.md`，梳理任务编排各节点的功能开发需求
2. 检索路由文档中的所有高层能力，将可用的到开发工作流中
   1. 根据高层能力的应用情景描述，初步判断是否有可以加入执行工作流的能力
   2. 如果判断能力可用，根据路由文档提供的路径，阅读对应的能力注册表，进一步确认是否可用
   3. 调用执行脚本，新建能力使用任务，执行脚本会反馈能力是否可用
   4. 如果能力允许使用，记录脚本反馈的键值，并将高层能力加入备选清单
   5. 重复上述步骤，直至检索完所有的高层能力
   6. 根据任务编排需求和能力清单，生成合适的开发执行链
3. 梳理剩余的功能开发需求，遍历检索路由文档中的低层能力，过程可参考步骤3的高产能力检索
4. 确认最终的开发执行链以及能力触发时机
5. 根据工作流完成任务编排，在需要使用能力是，调用执行脚本完成工具调用
...
3. AI调用执行脚本，检查能力是否可用，并缓存输入信息，得到key值
4. 被选中的能力被AI加入任务编排对应的执行链
5. AI根据工作流顺序，在需要调用能力时，通过key值调用执行脚本
6. 执行脚本按照能力注册表返回输出，AI继续工作流
...
从AI任务编排的视角出发，AI只需要通过注册信息判断工具/能力是否可以加入执行链，而不需要了解具体的调用细节。模板将面向AI（或人类）开发者，提供统一的工具调用脚本，起到以下几个方面的作用：
- 提供统一的操作、查询、以及输出 接口，AI只需要按照要求提供输入，并且可以预期得到的输出内容和格式
- 自动检查约束条件、触发护栏机制，判断是否可以执行。如果工具无法执行，则提前告知AI，避免将无效的工具加入执行链
- 自动加载默认参数、配置等信息，AI无需阅读调用细节，只需要判断工具的可用性
- 记录调用需求，脚本会自动记录AI的调用请求和执行请求，AI不用分心写入调用意图，而意图信息可以帮助完善路由文档
- 简化AI执行操作：直接脚本提供的主键调用执行，就可以完成按照缓存需求调用工具，不需要再次检验条件、输入信息等

当AI希望将某个能力加入到执行链条中时，需要先调用模板提供的全局唯一的执行脚本，告知能力和输入。执行脚本将解析输入信息，找到对应的工具，并检查工具是否具备执行条件。如果无法使用，脚本会告知AI工具当前不具备使用条件。如果可以使用，脚本则会缓存输入信息、加载配置信息、默认参数等必要信息，形成一个待执行的任务，并将任务的键值返回给AI，后续AI仅需要告诉执行脚本主键，脚本会自动调用工具，并给出输出。

- 新增任务：公共接口，需要接受规范的输出
  - 给定能力id、以及能力要求格式的输入
  - 会生成一个新的key，创建文件夹`/.system/implement/<key>/`和文档`/.system/implement/<key>/tool_call.md`
  - 该接口起到检查和缓存的作用，不会执行工具实现
- 工具调用：公共接口，接受两种形式的输入
  - 键值：通过键值查询缓存信息，根据配置等信息找到工具，传入输入信息，必要时串接流程（如交互输入）
  - 输入：通过给定调用能力的id、以及能力要求格式的输入，执行脚本直接进行能力调用，不会创建任务
- 检查能力的可用性：
  - 调用能力注册表中包含的护栏机制
  - 判断输入信息是否合法
  - 判断是否触发了限制条件
  - 判断是否需要手动确认
- 反馈能力的可用性：
  - 对于不可用的能力：告知能力不可用，并告知不可用的原因
  - 对于可用的能力：告知能力可用，并给出任务对应的键值
- 缓存信息
  - 当新增任务接口被调用时，缓存生成的键值以及相关输入，并关联键值和输入 
  - 在工具调用完成后，缓存工具的输出
- 信息查询和修改：公共接口，用来确认和修改任务相关输入
  - 通过任务主键来查询当前的输入
  - 通过任务主键、字段名称、修改值进行修改；允许批量修改
- 任务的删除：任务是具有生命周期的，以下几种情况，缓存脚本将会删除任务相关的缓存内容和对应的主键，
  - 执行脚本返回工具被调用的信息
  - 任务存在时间超过60分钟
  - 缓存被释放
- 记录信息：对于任务形式的调用（有键值的能力使用），对于脚本需要记录的以下几类信息
  - 新增任务后，询问调用者“为什么需要调用这个工具”，将调用意图写入文档`tool_call.md`，创建意实际执行工具的字段（使用占位符）
  - 接到执行指令后，将调用的能力、实际执行的工具写入`tool_call.md`
  - 执行完成后，将执行是否成功、执行错误原因等信息写入`tool_call.md`
  - 给出输出后，要求AI检查输出结果，并给出是否符合预期的反馈，将反馈信息写入`tool_call.md`

脚本执行低层能力通常比较清晰，直接根据配置信息调用脚本等底层实现即可。执行高层能力时的情况要更为复杂，几个比较常见的问题包括：
- 高层能力通常包含多个低层能力，那么就要求执行脚本有嵌套执行的能力
- 高层能力可能需要调用不属于项目的功能（特别是agent类的能力），执行脚本需要具备完善的基础功能（如mcp、api调用等）
- 脚本执行过程中可能需要交互类输入，在AI调用脚本的情景中，需要规范是由人给出输入还是由AI自行给出，并明确输入接口和形式
...
指向性调用的形式包括手动调用或给出AI明确的要求指令两种。不论哪种，都可以通过以下方式直接调用能力池中的能力：
- 按照规范流程，先调用缓存脚本，确认工具可以使用后，根据提供的key调用执行脚本
- 阅读能力对应的注册表，按照输入信息规范，直接调用执行脚本；该方法不会产生调用记录
- 直接调用能力池中的工具，该方法将不会产生任何记录和上下文信息
